...
- 5/1/2022: Talker/80 noise reduction quick hack: [Talker/80 Noise Reduction](https://youtu.be/PXlzUbNzO18) If you are using the speaker & built-in PAM amplifier, depending on your speaker, you might get quite a bit of noise. There is an easy fix for that - just pull the Op amp! The op amp is only needed for line out. And line out and speaker are mutually exclusive anyways. So - Op amp is needed for line out only, else it is better to just pull it. All ICs are socketed, so this is easy to do. 
